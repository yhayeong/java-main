...
//		method2(fighter);
	...
	/*
	 다중상속의 장점 :
	  ...
	  상속을 많이 하다보면 
	  클래스1과 클래스2만 사용가능한 공통기능을 만들고 싶을 수도 있다
	  
	  클래스는 다중상속이 안되니까
	  다른 클래스1,2의 기능을 같이 가질 수 없다
	  
	  상속관계로 묶이지 않은 클래스끼리의 관계를 만들고싶을때 
	  인터페이스를 껍데기로만 만들어두고 그것을 상속했는지 여부를 통해서 특정 클래스들을 공통적인 그룹으로 다룰 수 있게 된다
	  
	 
	 즉 클래스a와 클래스b는 같은 부모를 상속하지 않으면서, 그들끼리의 공통점을 가지고 있을 수 있다(어떤 메소드f를 둘다 사용할수있게할수있다)
	 추상메소드F를 가지는 인터페이스C를 만들어둔다
